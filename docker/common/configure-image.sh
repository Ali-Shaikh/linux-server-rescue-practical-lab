...
install -d -m 0755 /etc/sudoers.d /etc/rescue-web /var/lib/cloudsprocket-lab
...
chmod 0755 /usr/local/bin/rescue-cpu-hog
...
chmod 0644 /etc/systemd/system/rescue-web.service \
  ...
  /etc/systemd/system/rescue-cpu-hog.service \
  /etc/systemd/system/rescue-upstream-check.service
chmod 0644 /etc/rescue-web/config.json /etc/rescue-web/config.json.last-known-good
