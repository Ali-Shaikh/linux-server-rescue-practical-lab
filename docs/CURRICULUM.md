...
Status: thirteen complete incident slices in build, 2026-07-16.
...
| 2 | Capacity | Full filesystem, deleted-open file, exhausted inodes, backup sprawl | Docker | All four capacity incidents implemented |
...
Version 0.1 originally targeted three complete incidents:
...
That target is complete. The current alpha also includes:

- `04-permission-denied`: service identity and directory access diverge.
...
- `12-inode-exhaustion`: empty stale session files consume every inode while
  filesystem blocks remain available for application data.
- `13-backup-sprawl`: scheduled local backups have no retention policy, fill
  a bounded application filesystem and keep returning after superficial cleanup.
